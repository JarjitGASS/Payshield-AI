      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-lg">
            ...
            {step === 1 ? (
              ...
              >
                <div className="flex flex-col">
                  ...
                  <input name="username" type="text" required className="p-2 border rounded-lg text-black focus:ring-2 focus:ring-blue-500 outline-none" placeholder="johndoe123" />
                </div>
                <div className="flex flex-col">
                  <label className="text-sm font-semibold text-gray-600 mb-1">Password</label>
                  <input name="password" type="password" required className="p-2 border rounded-lg text-black focus:ring-2 focus:ring-blue-500 outline-none" placeholder="••••••••" />
                </div>
                <div className="flex flex-col">
                  <label className="text-sm font-semibold text-gray-600 mb-1">Confirm Password</label>
                  <input name="confirmPassword" type="password" required className="p-2 border rounded-lg text-black focus:ring-2 focus:ring-blue-500 outline-none" placeholder="••••••••" />
                </div>
                <button 
                  type="button" 
                  onClick={() => setStep(2)}
              ...
              >
                <div className="flex flex-col">
                  ...
                  <input name="nik" type="text" required className="p-2 border rounded-lg text-black" placeholder="1234567890123456" />
                </div>
                <div className="flex flex-col">
                  <label className="text-sm font-semibold text-gray-600 mb-1">Nama Lengkap</label>
                  <input name="fullname" type="text" required className="p-2 border rounded-lg text-black" placeholder="John Doe" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col">
                    <label className="text-sm font-semibold text-gray-600 mb-1">Tempat Lahir</label>
                    <input name="pob" type="text" required className="p-2 border rounded-lg text-black" placeholder="Jakarta" />
                  </div>
                  <div className="flex flex-col">
                    <label className="text-sm font-semibold text-gray-600 mb-1">Tanggal Lahir</label>
                    <input name="dob" type="date" required className="p-2 border rounded-lg text-black" />
                  </div>
                </div>
                <div className="flex flex-col">
                  <label className="text-sm font-semibold text-gray-600 mb-1">Jenis Kelamin</label>
                  <select name="gender" required className="p-2 border rounded-lg text-black">
                ...
                <div className="flex flex-col">
                  ...
                  <input name="file" type="file" accept="image/*" required className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:bg-blue-50 file:text-blue-700" />
                </div>
        ...
        {isError !== null && (
          <div className={`mt-6 p-4 rounded-lg text-center ${isError ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
            {message}
            <p>{analysis!=null?JSON.stringify(analysis):null}</p>
          </div>
        )}
      </div>
    </div>
